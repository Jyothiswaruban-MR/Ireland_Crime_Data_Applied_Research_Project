...
DEFAULT_DIVISION = "All divisions"
DEFAULT_OFFENCE = "All offences"
...
if show_filters:
    ...
    if region_sel != "All regions":
        div_options = ["All divisions"] + sorted(
            rec_df.loc[rec_df["region"] == region_sel, "division"]
            .dropna()
            .unique()
        )
    else:
    ...
    division_sel = st.sidebar.selectbox(
        "Division",
        div_options,
        key="division",
    )

    offence_sel = st.sidebar.selectbox(
        "Offence",
        offence_options,
        key="offence",
    )
    ...
    # Defaults when filters are hidden
...
def generate_insights(
    ...
    if not by_off.empty:
        ...
        insights.append(
            f"The most common offence was **{top['offence']}**, accounting for "
            f"**{share:.1f}%** of incidents."
        )
    ...
    if region == "All regions" and len(by_reg) >= 2:
        ...
        insights.append(
            f"**{r1['region']}** recorded the highest incidents "
            f"({r1['incidents']:,}), "
...
if page == "Overview":
    ...
    # --- Crimes by Region (last year in range) ---
...
# =========================
    ...
    sort_order = st.radio(
        "Sort order",
        ["Highest incidents", "Lowest incidents"],
        horizontal=True,
    )
    ...
    chart_station = (
        ...
        .mark_bar()
        .encode(
            x=alt.X("garda_station:N", sort="-y"),
            y=alt.Y("incidents:Q"),
...
# =========================
    ...
    # -------------------------
    # Division breakdown (Filtered Scope)
    # -------------------------
...
# =========================
    ...
    st.markdown(
        ...
        "Results are illustrative and intended for analytical discussion."
    )
    ...
    with col1:
        ...
        if forecast_region_df is None:
            st.info("Region-level forecast data not available.")
        else:
            cols = forecast_region_df.columns.str.lower()

            year_col = forecast_region_df.columns[cols == "year"][0]
            reg_col = forecast_region_df.columns[cols.str.contains("region")][0]

            true_col = next(
                (c for c in forecast_region_df.columns if "true" in c.lower() or "actual" in c.lower()),
                "total_incidents",
            )
            pred_col = next(
                (c for c in forecast_region_df.columns if "pred" in c.lower()),
                "prediction",
            )
            ...
            sel_reg = st.selectbox(
                ...
                key="forecast_region_sel",
            )

            sub = forecast_region_df[forecast_region_df[reg_col] == sel_reg]
            ...
            chart_reg_forecast = (
                ...
                .encode(
                    ...
                    y="Incidents:Q",
                    color="Series:N",
                    tooltip=[year_col, "Series", "Incidents"],
                )
                .properties(height=300)
            )
    ...
    # -------------------------
    # Division-level Forecast
    # -------------------------
    with col2:
        st.markdown("### Division-level Forecast")

        if forecast_div_df is None:
            st.info("Division-level forecast data not available.")
        else:
            cols = forecast_div_df.columns.str.lower()

            year_col = forecast_div_df.columns[cols == "year"][0]
            div_col = forecast_div_df.columns[cols.str.contains("division")][0]

            true_col = next(
                (c for c in forecast_div_df.columns if "true" in c.lower() or "actual" in c.lower()),
                "total_incidents",
            )
            pred_col = next(
                (c for c in forecast_div_df.columns if "pred" in c.lower()),
                "prediction",
            )
            ...
            sel_div = st.selectbox(
                ...
                key="forecast_div_sel",
            )

            sub = forecast_div_df[forecast_div_df[div_col] == sel_div]
